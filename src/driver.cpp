...
int GetRandom(int Low, int High) { return rand() % (High - Low + 1) + Low; }
...
unsigned ConstantHash(const char*, unsigned) { return 1; }
...
void TestALot(HashData* phd, HashData* shd) {
  ...
  try {
    ...
    key = keys[0];
    cout << endl
         << "Finding key: " << key << " (" << phf(key, ht.GetStats().TableSize_)
         << ")" << endl;
    try {
    ...
    try {
      ...
      cout << endl
           << "**** Something bad happened finding in TestALot" << endl
           << endl;
    }
    ...
    try {
      ...
      cout << endl
           << "**** Something bad happened inserting in TestALot" << endl
           << endl;
    }
    DumpStats<T>(ht);
  ...
  try {
    ...
    for (unsigned i = 0; i < size; i++) {
      ...
      cout << "Inserting key: " << key << " ("
           << phf(key, ht.GetStats().TableSize_) << ":"
...
void TestSimpleInsert1() {
  cout << endl
       << "==================== TestSimpleInsert1 ====================" << endl;

  typedef Person* T;
  OAHashTable<T> ht(
    OAHashTable<T>::OAHTConfig(5, UHash, RSHash, .95, 2.0, PACK, 0)
  );
  try {
...
void TestSimpleInsert2() {
  cout << endl
       << "==================== TestSimpleInsert2 ====================" << endl;

  typedef Person* T;
  OAHashTable<T> ht(
    OAHashTable<T>::OAHTConfig(11, UHash, RSHash, .95, 2.0, PACK, 0)
  );
  try {
...
void TestInsertConstant() {
  ...
  try {
    ...
    cout << endl
         << "**** Something bad happened inserting in TestSimpleInsert" << endl
         << endl;
  }
}
...
void TestSimpleDeletePresent() {
  ...
  typedef Person* T;
  OAHashTable<T> ht(
    OAHashTable<T>::OAHTConfig(11, PJWHash, NULL, 1.0, 2.0, PACK, Dispose)
  );
  try {
    for (unsigned i = 0; i < 11; i++) {
      Person* person = PersonRecs[i];
    ...
    cout << endl
         << "**** Something bad happened inserting in TestSimpleDeletePresent"
         << endl
         << endl;
  }
}
...
void TestSimpleDeleteMissing() {
  ...
  try {
    ...
    cout << endl
         << "**** Something bad happened inserting in TestSimpleDeleteMissing"
         << endl
         << endl;
  }
  DumpStats<T>(ht);
...
void TestSimpleDispose() {
  cout << endl
       << "==================== TestSimpleDispose ====================" << endl;

  double lf = 0.667;
  typedef char* T;
  OAHashTable<T> ht(
    OAHashTable<T>::OAHTConfig(11, PJWHash, NULL, lf, 2.0, PACK, FreeSimpleRec)
  );
  for (unsigned i = 0; i < 11; i++) {
    char* ID = new char[8];
    sprintf(ID, "%07d", i);
    ...
    try {
      ...
      cout << endl
           << "**** Something bad happened inserting in TestSimpleDispose"
           << endl
           << endl;
      break;
    }
...
void TestSimpleGrow1() {
  cout << endl
       << "==================== TestSimpleGrow1 ====================" << endl;

  double lf = 0.90;
  typedef char* T;
  OAHashTable<T> ht(
    OAHashTable<
      T>::OAHTConfig(5, SimpleHash, NULL, lf, 2.0, PACK, FreeSimpleRec)
  );
...
void TestSimpleGrow2() {
  cout << endl
       << "==================== TestSimpleGrow2 ====================" << endl;

  double lf = 0.90;
  typedef char* T;
  OAHashTable<T> ht(
    OAHashTable<T>::OAHTConfig(5, UHash, RSHash, lf, 2.0, MARK, FreeSimpleRec)
  );
  for (unsigned i = 0; i < 30; i++) {
    char* ID = new char[8];
    sprintf(ID, "%07d", i);
...
void TestSimpleMarkPack(
  ...
  try {
    ...
    // cout << endl << "errno: " << e.code() << ", " << e.what() << endl <<
    // endl; cout << endl << "**** Caught exception deleting non-existent item.
    // (SimpleTest1) ****" << endl << endl;
  } catch (...) {
    cout << endl
         << "**** Something bad happened inserting in " << test << endl
         << endl;
  }
}
...
int main(int argc, char** argv) {
  ...
  // OAHashTable<float *> ht(config); // <----if this is changed to <float *>
  // the program
  ...
  switch (test_num) {
    case 1: TestALot(&HashingFuncs[SIMPLE], &HashingFuncs[NONE]); break;
    case 2: TestSimpleGrow1(); break;
    case 3: TestSimpleDeletePresent(); break;

    case 4: TestSimpleDeleteMissing(); break;

    case 5: TestSimpleDispose(); break;
    ...
    case 6:
      ...
      // ***********************
    case 7: TestALot(&HashingFuncs[PJW], &HashingFuncs[RS]); break;

    case 8: TestSimpleInsert1(); break;

    case 9: TestSimpleInsert2(); break;

    case 10: TestSimpleGrow2(); break;
